...
Example — fetch the latest Versioniq SBOM:

```bash
curl -sL https://github.com/ConductionNL/versioniq/releases/latest/download/sbom.cdx.json | jq .
```

Example — fetch the SBOM for a specific historical release:

```bash
curl -sL https://github.com/ConductionNL/versioniq/releases/download/v1.0.0/sbom.cdx.json | jq .
```
