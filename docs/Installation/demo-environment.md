...
This page gets a working Versioniq running on your own machine in two commands. You end with app version tracking across a fleet.
...
```bash
curl -fsSLO https://raw.githubusercontent.com/ConductionNL/versioniq/development/versioniq-compose.yaml
```
...
==> installing versioniq <version>
==> apps present: integriq openregister thematiq versioniq
```
...
```bash
curl -s http://localhost:8619/status.php
```
...
| What | Where |
| --- | --- |
| **Versioniq** | [http://localhost:8619/apps/versioniq/](http://localhost:8619/apps/versioniq/) |
| Admin interface | [http://localhost:8619](http://localhost:8619) — `admin` / `admin` |
...
| `versioniq` | The app this page is about. |

That OpenRegister dependency is **not declared** in `appinfo/info.xml` — no app in the fleet declares an `<app>` dependency — so nothing stops the App Store from installing versioniq without it. It would then load, find no register to attach to, and show you an empty app rather than an error. The compose file encodes the dependency the manifest does not.
...
curl -s -o /dev/null -w '%{http_code}\n' -u admin:admin -L "http://localhost:8619/apps/versioniq/"
...
curl -s -u admin:admin "http://localhost:8619/apps/openregister/api/registers" | head -c 300
```
...
```bash
# Stop, keep the data
docker compose -f versioniq-compose.yaml down

# Stop and delete everything, including the database
docker compose -f versioniq-compose.yaml down -v
```
...
**Everything returns 404 or a maintenance page after a restart.** Nextcloud is waiting for an upgrade. Run `docker compose -f versioniq-compose.yaml exec -u www-data nextcloud php occ upgrade`.
