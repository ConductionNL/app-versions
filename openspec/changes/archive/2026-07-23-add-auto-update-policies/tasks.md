...
- **Acceptance criteria**:
  ...
  - Pin skip — `add-version-pinning` was already merged to development by the time this task was implemented, so `PinStore` is wired in directly (concrete DI) rather than via an optional-service/`class_exists` lookup; the flexible-build-order guard is unnecessary now that the dependency is guaranteed present
