...
test.describe('occ CLI', () => {
	...
	test('lists versions in human-readable form', async () => {
		const { code, stdout } = await occ('versioniq:versions', FIXTURE_APP, `--source=${FIXTURE_SOURCE}`)
		expect(code).toBe(0)
	...
	test('lists versions as JSON', async () => {
		const { code, stdout } = await occ('versioniq:versions', FIXTURE_APP, `--source=${FIXTURE_SOURCE}`, '--json')
		expect(code).toBe(0)
	...
	test('installs a specific version reproducibly', async () => {
		const { code, stdout } = await occ('versioniq:install', FIXTURE_APP, '1.0.1', `--source=${FIXTURE_SOURCE}`, '--json')
		expect(code).toBe(0)
	...
	test('refuses a downgrade without --allow-downgrade, then proceeds with it', async () => {
		...
		const refused = await occ('versioniq:install', FIXTURE_APP, '1.0.0', `--source=${FIXTURE_SOURCE}`, '--json')
		...
		const ok = await occ('versioniq:install', FIXTURE_APP, '1.0.0', `--source=${FIXTURE_SOURCE}`, '--allow-downgrade', '--json')
	...
	test('a dry run leaves the instance untouched', async () => {
		...
		const { stdout } = await occ('versioniq:install', FIXTURE_APP, '1.1.0', `--source=${FIXTURE_SOURCE}`, '--dry-run', '--json')
