...
test.describe('forge installs (fixture-backed)', () => {
	...
	test('installs a specific newer version from the forge', async ({ page }) => {
	...
	test('records the observed SHA-256 on first successful install (TOFU)', async ({ page }) => {
		await installFixture(page, '1.0.1')
		const binding = JSON.parse((await appConfigValue(page, `source.${FIXTURE_APP}`)) ?? '{}')
		expect(binding.sha256?.['1.0.1'], 'digest recorded for the installed version').toMatch(/^[a-f0-9]{64}$/)
	})

	test('a rewritten release fails closed against the recorded digest', async ({ page }) => {
		...
		expect(JSON.stringify(body).toLowerCase()).toMatch(/sha|checksum|digest|integrity/)
	})

	test('acceptNewSha installs a rewritten release the default refuses', async ({ page }) => {
		...
		expect(JSON.stringify(refused.body).toLowerCase()).toMatch(/sha|checksum|digest|integrity/)

		// Acknowledged: the same rewritten release installs.
		const accepted = await installFixture(page, '1.0.1', { acceptNewSha: true })
		expect(accepted.body.installStatus).toBe('installed')
		expect(accepted.body.installedVersion).toBe('1.0.1')
	})

	test('a downgrade is refused without acknowledgement, then proceeds with it', async ({ page }) => {
	...
	test('a rate-limited forge surfaces an error in the version list', async ({ page }) => {
		...
		expect(data.error ?? '', 'an error should be reported, not silent zero versions').not.toBe('')
		expect(data.availableVersions ?? []).toHaveLength(0)
	})

	test('offline artifact cache serves a rollback when the release is gone', async ({ page }) => {
		...
		const { body } = await installFixture(page, '1.0.1', { allowDowngrade: true })
		expect(body.installStatus, 'cache should serve the vanished release').toBe('installed')
		expect(body.servedFromCache).toBe(true)
	})

	test('offline cache badge shows in the version picker after an install', async ({ page }) => {
		await installFixture(page, '1.0.1')
		await openSettings(page)
		const card = page.locator('article').filter({ has: page.getByText(FIXTURE_APP, { exact: true }) }).first()
