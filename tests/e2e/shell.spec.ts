...
test.describe('admin settings shell', () => {
	test('renders every capability tab in the settings section', async ({ page }) => {
		...
		await expect(tablist.getByRole('tab', { name: 'Apps', exact: true })).toHaveAttribute('aria-selected', 'true')
	})
	...
	test('every control exposes an accessible name (WCAG 4.1.2)', async ({ page }) => {
		await openSettings(page)
	...
	test('the app list loads installed apps', async ({ page }) => {
		...
		await expect(panel.getByText('dashboard', { exact: true }).first()).toBeVisible()
		await expect(panel.getByRole('button', { name: 'Choose app' }).first()).toBeVisible()
	})

	test('unauthenticated callers cannot reach the settings API', async ({ playwright, baseURL }) => {
