...
test.describe('PAT validation & lifecycle', () => {
	...
	test('a classic GitHub PAT with only repo scope is accepted', async ({ page }) => {
		// github base points at the fixture; /user returns X-OAuth-Scopes: repo.
		const res = await addPat(page, { forge: 'github', label: 'repo-scope', token: 'ghp_scope-repo-000000000000000000' })
		expect(res.ok(), 'repo-only classic PAT accepted').toBeTruthy()
	})

	test('a classic GitHub PAT with over-broad scope is rejected', async ({ page }) => {
		const res = await addPat(page, { forge: 'github', label: 'admin-scope', token: 'ghp_scope-admin-00000000000000000' })
		expect(res.ok(), 'over-broad classic PAT rejected').toBeFalsy()
		expect(JSON.stringify((await res.json())?.ocs?.data ?? {}).toLowerCase()).toMatch(/scope|admin|write/)
	})

	test('a token expiry is captured from the forge response', async ({ page }) => {
		await addPat(page, { forge: 'github', label: 'expiring-token', token: 'ghp_expires-000000000000000000000' })
		const exp = (await sql("SELECT expires_at FROM oc_app_versions_pats WHERE label='expiring-token'")).trim()
		expect(exp, 'expiry parsed and stored').toMatch(/2026-08-15/)
	})

	test('the stored token is encrypted at rest, never plaintext', async ({ page }) => {
		await addPat(page, { forge: 'codeberg', label: 'enc', token: 'codeberg-plaintext-marker-zzz999' })
		const enc = (await sql("SELECT encrypted_token FROM oc_app_versions_pats WHERE label='enc'")).trim()
		expect(enc.length, 'a value is stored').toBeGreaterThan(0)
		expect(enc, 'not the plaintext token').not.toContain('codeberg-plaintext-marker-zzz999')
	})

	test('a PAT-gated private repo is reachable with a matching token, not without', async ({ page }) => {
	...
	test('an expired PAT is skipped, falling back to the unauthenticated path', async ({ page }) => {
		await resetFixtureApp(page)
		await fixtureControl(page, 'repo', { repo: 'fixtureowner/fixtureapp', requireAuth: true })
	...
	test('deleting a user sweeps their PATs', async () => {
		...
		expect(Number(await sql("SELECT count(*) FROM oc_app_versions_pats WHERE owner_uid='pat-sweep-user'")), 'PATs swept on user deletion').toBe(0)
	})
})
