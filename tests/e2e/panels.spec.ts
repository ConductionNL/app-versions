...
test.describe('capability panels', () => {
	test('history explains itself and lists entries newest-first', async ({ page }) => {
		...
		if (await table.isVisible()) {
			for (const header of ['When', 'Who', 'App', 'Operation', 'Status']) {
				await expect(table.getByRole('columnheader', { name: header })).toBeVisible()
			}
		}
	})

	test('artifact cache reports its size and disables clearing when empty', async ({ page }) => {
		await openSettings(page)
		const panel = await openTab(page, 'Artifact cache')

		await expect(panel.getByRole('heading', { name: 'Release artifact cache' })).toBeVisible()
	...
	test('tokens panel lists PATs and never leaks a secret', async ({ page }) => {
		await openSettings(page)
		const panel = await openTab(page, 'Tokens')

		await expect(panel).toBeVisible()
		// With no tokens configured the panel must say so rather than render blank.
		await expect(panel.getByText('No tokens configured.').or(panel.locator('li')).first()).toBeVisible()
	...
	test('trusted sources shows the default forge allowlist', async ({ page }) => {
		await openSettings(page)
	...
	test('sources panel offers binding an app to a forge', async ({ page }) => {
		...
		await expect(panel).toBeVisible()
		await expect(panel.getByRole('button', { name: 'Bind source' })).toBeVisible()
	})
})
