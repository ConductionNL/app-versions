...
test.describe('background jobs', () => {
	...
	async function setPolicy(page: import('@playwright/test').Page, level: string) {
		await page.request.put(`/ocs/v2.php/apps/versioniq/api/app/${FIXTURE_APP}/policy?format=json`, {
	...
	test.afterEach(async ({ page }) => {
		...
		await occ('config:app:delete', 'versioniq', `auto_attempt.${FIXTURE_APP}`)
	...
	test('a patch policy applies the highest same-minor release, and notifies', async ({ page }) => {
		...
		await occ('config:app:set', 'versioniq', 'auto_update_enabled', '--value', '1')
		await occ('config:app:set', 'versioniq', 'auto_update_window', '--value', '00:00-23:59')
		...
		const subj = await sql("SELECT subject FROM oc_notifications WHERE app='versioniq'")
	...
	test('a pinned app is skipped by the auto-update job', async ({ page }) => {
		...
		await page.request.put(`/ocs/v2.php/apps/versioniq/api/app/${FIXTURE_APP}/pin?format=json`, {
	...
	test('the job is a no-op while the kill switch is off', async ({ page }) => {
		await setPolicy(page, 'all')
		await occ('config:app:set', 'versioniq', 'auto_update_enabled', '--value', '0')
	...
	test('a failed auto-update attempt is not retried', async ({ page }) => {
		...
		expect(await sql("SELECT subject FROM oc_notifications WHERE app='versioniq'")).toContain('auto_update_failure')
	})
	...
	test('the expiry job warns once for a token nearing expiry', async ({ page }) => {
		...
		await page.request.post('/ocs/v2.php/apps/versioniq/api/pats?format=json', {
			headers: { 'OCS-APIRequest': 'true', 'Content-Type': 'application/json' },
		...
		// cleanup
		await page.request.delete(`/ocs/v2.php/apps/versioniq/api/pats/${id}?format=json`, {
			headers: { 'OCS-APIRequest': 'true' },
		}).catch(() => undefined)
	})
	...
	test('the prune job removes entries older than the retention window', async ({ page }) => {
		...
		await occ('config:app:set', 'versioniq', 'audit_retention_days', '--value', '365')
