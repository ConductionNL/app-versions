...
test.describe('install effects', () => {
	...
	test('a successful install updates the last-known-good record', async ({ page }) => {
		await installFixture(page, '1.0.1')
		const lkg = JSON.parse((await appConfigValue(page, `lkg.${FIXTURE_APP}`)) ?? '{}')
		expect(lkg.version).toBe('1.0.1')
		expect(lkg.recordedAt).toBeTruthy()
	})

	test('a failed install preserves the last-known-good record', async ({ page }) => {
		await installFixture(page, '1.0.1') // lkg = 1.0.1
		await fixtureControl(page, 'repo', {
			repo: 'fixtureowner/fixtureapp',
			releases: [{ tag: 'v2.0.0', asset: 'fixtureapp-wrongid.tar.gz', sha: true }],
		})
	...
	// --- artifact-cache ----------------------------------------------------
	test('a successful install populates the artifact cache', async ({ page }) => {
		await installFixture(page, '1.0.1')
		const summary = await cache(page)
		const app = (summary.apps ?? []).find((a: any) => a.appId === FIXTURE_APP)
		expect(app, 'fixture app has a cache entry').toBeTruthy()
		expect(app.versions).toContain('1.0.1')
		expect(summary.totalSizeBytes ?? summary.totalSize ?? 0).toBeGreaterThan(0)
	})
	...
	test('a forge release body is returned as the version changelog', async ({ page }) => {
		const data = await versions(page)
		const entry = data.availableVersions.find((v: any) => v.version === '1.0.1')
		expect(entry.changelog ?? '').toContain('Fixture release notes for v1.0.1')
	})
