...
test.describe('faults, diffs and cache integrity', () => {
	...
	test('a downgrade names the migration steps the target lacks', async ({ page }) => {
		...
		expect(JSON.stringify(data.orphanedMigrations ?? [])).toContain('Version1020')
	})

	test('a downgrade between releases with the same migrations reports no drift', async ({ page }) => {
	...
	test('a finalize-phase failure reverts and reports installed-but-broken', async ({ page }) => {
		await fixtureControl(page, 'repo', {
			repo: 'fixtureowner/fixtureapp',
			releases: [{ tag: 'v1.3.0', asset: 'fixtureapp-1.3.0.tar.gz', sha: true }],
		})
		...
		expect(JSON.stringify(body).toLowerCase()).toMatch(/finaliz|revert|broken|migrat/)
		expect((await occ('config:app:get', FIXTURE_APP, 'installed_version')).trim()).toBe('1.0.0')
	})
	...
	test('a tampered cached artifact is discarded, not installed', async ({ page }) => {
