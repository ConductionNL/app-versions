...
import { appConfigValue, openSettings, openTab } from './helpers'
...
test.describe('auto-update policies', () => {
	...
	test('automation is off by default and says so', async ({ page }) => {
		...
		await expect(page.getByTestId('auto-update-kill-switch')).not.toBeChecked()
		// Default window is advertised.
		await expect(page.getByTestId('auto-update-window')).toHaveValue(/01:00-05:00/)
	})

	test('an invalid maintenance window is rejected before saving', async ({ page }) => {
		await openSettings(page)
		await openTab(page, 'Apps')

		await page.getByTestId('auto-update-window').fill('nonsense')
		await expect(page.getByTestId('auto-update-window-error')).toBeVisible()
		await expect(page.getByTestId('auto-update-settings-save')).toBeDisabled()
	})

	test('enabling automation with a valid window persists server-side', async ({ page }) => {
		await openSettings(page)
		await openTab(page, 'Apps')
		...
		await expect.poll(async () => appConfigValue(page, 'auto_update_enabled'), {
			message: 'kill switch should persist',
			timeout: 20_000,
		}).toBe('1')
		expect(await appConfigValue(page, 'auto_update_window')).toBe('23:00-03:00')
		...
		await page.getByTestId('auto-update-settings-save').click()
		await expect.poll(async () => appConfigValue(page, 'auto_update_enabled'), { timeout: 20_000 })
			.not.toBe('1')
	})
	...
	test('a per-app policy is persisted and badged', async ({ page }) => {
		...
		await expect.poll(async () => appConfigValue(page, `policy.${APP}`), {
