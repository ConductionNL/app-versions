...
test.describe('version management', () => {
	...
	test.describe('with the fixture forge', () => {
		...
		test('re-binding overwrites the previous binding', async ({ page }) => {
			...
			expect(JSON.parse((await appConfigValue(page, `source.${FIXTURE_APP}`)) ?? '{}').repo).toBe('other-repo')
			await bind('fixtureapp')
			expect(JSON.parse((await appConfigValue(page, `source.${FIXTURE_APP}`)) ?? '{}').repo).toBe('fixtureapp')
		})
		...
		test('a one-off source query does not change the stored binding', async ({ page }) => {
		...
		test('a silent dry run reports its outcome without changing the version', async ({ page }) => {
			...
			const after = (await occ('config:app:get', FIXTURE_APP, 'installed_version')).trim()
			expect(after).toBe(before)
		})
		...
		test('an older version installs as a rollback (with acknowledgement)', async ({ page }) => {
	...
	test.describe('read-only surfaces', () => {
		test('the failure category drives the HTTP status (409 on a guarded downgrade)', async ({ page }) => {
			test.skip(!(await fixtureAvailable(page)), 'forge fixture not running')
			await resetFixtureApp(page)
			await installFixture(page, '1.1.0')
			const res = await page.request.post(
			...
			const meta = (await res.json())?.ocs?.meta ?? (await res.request?.() as never)
			void meta
		})

		test('a core/shipped app is flagged and not installable on its card', async ({ page }) => {
			await openSettings(page)
			const panel = await openTab(page, 'Apps')
			// `files` is a core-shipped app on any instance.
			const card = panel.locator('article').filter({ has: page.getByText('files', { exact: true }) }).first()
			await expect(card).toContainText('CORE')
			// Core apps carry no "Choose app" action — they cannot be version-managed.
			await expect(card.getByRole('button', { name: 'Choose app' })).toHaveCount(0)
		})
	})
