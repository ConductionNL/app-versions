...
import { fixtureAvailable, occ, openSettings, openTab } from './helpers'
...
test.describe('PAT management', () => {
	...
	test('a valid Codeberg token is accepted with an unverifiable-scope warning', async ({ page }) => {
		...
		expect(JSON.stringify(body).toLowerCase()).toMatch(/unverifi|scope|warning/)
	})
	...
	test('a created token is never exposed in plaintext by the API', async ({ page }) => {
		await addToken(page, { token: 'codeberg-secret-abcdef123456' })
		const list = await pats(page)
		expect(list.length).toBe(1)
		expect(JSON.stringify(list)).not.toContain('codeberg-secret-abcdef123456')
	...
	test('per-forge token-creation deeplinks are provided', async ({ page }) => {
	...
	test('the Tokens panel lists tokens redacted', async ({ page }) => {
		await addToken(page, { token: 'codeberg-panel-token-zzz', label: 'panel token' })
